...
public class TuGraph
{
	...
	private int gHeight = 0; // height of the graph
	private int gWidth = 0; // width of the graph
	public int xCoord; // x-coordinate of the current position
	private int yCoord; // x-coordinate of the current position
	private int[] stateNumber;
	private int[] colNumber;
	private int[] x;
	private int[] yc;
	private int lastX;
	private HashMap<String, PenAttributes<Color, Float>> pen = new HashMap<String, PenAttributes<Color, Float>>();
	private Font font, font_col;
	private Color brush, brush_col, brush_sel;
	...
	public void addTransition(double strtPos, int tokenId, double destPos)
	{
		...
		ArrayList<TransCell> trans = transitions.get(isrc);
	...
	public void computeXYcoord(JPanel mftPanel, double selectAllAnnotationsAtPosition, int y)
	{
		...
		for (int iCol = 0; iCol < stateNumber.length; iCol++)
		{
			...
			int statePos = (int) (100 * stPositions.get(iState));
	...
	public void computeCurrentFrameX(double selectAllAnnotationsAtPosition, JPanel mftPanel)
	{
		...
		for (int iCol = 0; iCol < stateNumber.length; iCol++)
		{
			...
			if (sp == selectAllAnnotationsAtPosition)
			{
				if (mftPanel.getComponentOrientation() == ComponentOrientation.RIGHT_TO_LEFT)
				{
					currentFrameX = ((x0 - 1) / mftPanel.getWidth() + 1) * mftPanel.getWidth();
				}
				else
				{
					currentFrameX = (x0 / mftPanel.getWidth()) * mftPanel.getWidth();
				}
			}
		}
		return;
	...
	public void computeDrawing(ArrayList<Object> annotations, JMftPanel mftPanel, Graphics2D g)
	{
		...
		for (int iCol = 0; iCol < nbOfStates; iCol++)
		{
			...
			int posi = (int) (100 * stPositions.get(si));

			for (int jCol = iCol + 1; jCol < nbOfStates; jCol++)
			{
				int sj = stateNumber[jCol];
				int posj = (int) (100 * stPositions.get(sj));
				if (posi > posj)
				{
					...
					si = stateNumber[iCol];
					posi = (int) (100 * stPositions.get(si));
				}
			}
		}
		...
		while (modified)
		{
			...
			{
				int iState = stateNumber[iCol];
				ArrayList<TransCell> trans = transitions.get(iState);
		...
		for (int iState = 0; iState < nbOfStates; iState++)
		{
			...
			for (int i = 0; i < trans.size(); i++)
			{
				...
				{
					int desI = (int) (100 * stPositions.get(trans.get(i).dest));
					String labI = (String) annotations.get(trans.get(i).tokenId);
					int desJ = (int) (100 * stPositions.get(trans.get(j).dest));
		...
		for (int iCol = 0; iCol < nbOfStates; iCol++)
		{
			...
			for (int iTrans = 0; iTrans < trans.size(); iTrans++)
			{
				...
				for (int jcol = iCol + 1; jcol < nbOfStates; jcol++)
				{
					...
					if ((int) (100 * stPositions.get(t.dest)) > (int) (100 * stPositions.get(jState)))
					{
						if (yc[jcol] < curry + 40)
							yc[jcol] = curry + 40;
					}
			...
			Graphics2D g)
	{
		...
		{
			int iState = stateNumber[iCol];
			int statePos = (int) (100 * ((stPositions.get(iState)) + 0.005));
			ArrayList<TransCell> trans = transitions.get(iState);
			...
			for (int ixref = 0; ixref < allXRefs.size(); ixref += 4)
			{
				...
				for (int jxref = ixref + 4; jxref < allXRefs.size(); jxref += 4)
				{
					...
					if (xrefi == xrefj)
					{
						...
						pt1 = new Point(((xendi)), yi);
						pt4 = new Point((xbegj), yj);
